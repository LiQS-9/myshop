...
public class EmailUtils {
	...
	private static MimeMessage createMsg(Session session, String myAccount, User user) {
		...
		try {
			 ...
			 * */
			message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(user.getUemail(), user.getUname(), "utf-8"));
			//3.3生成邮件主题
			message.setSubject("MyShop商城账号激活邮件","utf-8");
			String ip = Inet4Address.getLocalHost().getHostAddress();
			String url = "http://"+"192.168.58.157"+":8080/test02/user.do?method=active&c="+Base64Utils.encode(user.getUcode());
			//设置邮件正文 setContent 可以使用html标签
			message.setContent(user.getUname()+",你好<br>欢迎注册myshop商城! 请点击链接进行激活:<a href='"+url+"'>点击此处</a>","text/html;charset=utf-8");
			//设置邮件的发送时间 是立即发送
